...
class Realizacje {
    ...
    public function carGet($id) {
		...
		$sql = "SELECT c.*, cpt.symbol as waluta_symbol, cpt.nazwa as waluta_nazwa, cpo.symbol as pochodzenie, cpo.icon as ikona, cpo.opis_strona as opis,  cp.producer_name as producer, cm.model_name as model, cg.name as gearbox, cc.name as colour, cf.name as f, cb.name as body
	...
	public function carList($start = 0, $ile = 10, $order = 'default') {
		...
		switch($order) {
			...
			case 'name': $sql_order = 'cp.producer_name ASC, cm.model_name ASC'; break;
			case 'last': $sql_order = 'c.car_add DESC'; break;
			case 'sale': $sql_order = 'c.sale_date ASC'; break;
			case 'random': $sql_order = 'c.car_id'; /*$start = rand(0, 7500);*/ break;
			default: $sql_order = 'c.sale_date ASC';
		}
	...
	public function getFiltr() {
		...
		if (isset($this->filtr['sphinx']) && is_array($this->filtr['sphinx']) && count($this->filtr['sphinx']) > 0) {
			$prodSQL = implode(',', $this->filtr['sphinx']);
			$sql .= " AND c.car_id IN ($prodSQL) ";
		}
	...
	public function carProducersList($type = false, $ilosc = false, $order = false, $carId = null) {
		
		if($type) $where_type = " AND type = '".$type."' ";
		
		if($carId != null) {
			$sqlJoin = " LEFT JOIN autosalon_car c ON c.producer_id = cp.producer_id ";
			$where_type3 = ' AND c2.car_id IN ('.implode(',', $carId).') ';
			$where_type = ' AND c.car_id IN ('.implode(',', $carId).') ';
			$group = ' GROUP BY cp.producer_id ';
		} else {
			
		}
		
		if($ilosc) {
			$where_type2 .= ' HAVING ile > 15 ';
		} else {
			$where_type2 .= ' HAVING ile > 0 ';
		}
		...
		$sql = "SELECT cp.producer_name, cp.producer_id, (SELECT count(c2.car_id) FROM ".MyConfig::getValue("dbPrefix")."car c2 WHERE cp.producer_id = c2.producer_id ".$where_type3.") as ile FROM ".MyConfig::getValue("dbPrefix")."car_producer cp ".$sqlJoin." WHERE cp.type = '".$type."' " .$where_type.$group. " ".$where_type2." ORDER BY ".$order_type;
	...
	public function carModelsList($producer_list, $ile = true, $carId = null) {
		
		
		
		if($carId != null) {
			$sqlJoin = " LEFT JOIN autosalon_car c ON c.model_id = cm.model_id ";
			$where_type3 = ' AND c2.car_id IN ('.implode(',', $carId).') ';
			$where_type = ' AND c.car_id IN ('.implode(',', $carId).') ';
			$group = ' GROUP BY cm.model_id ';
		}
		
		if($ile) {
			$select_1 = " , (SELECT count(c2.car_id) FROM ".MyConfig::getValue("dbPrefix")."car c2 WHERE c2.model_id = cm.model_id ".$where_type3.") as ile";
			$where_1 = "HAVING ile > 0";
		}
		
		$sql = "SELECT cm.model_name, cm.model_id ".$select_1." FROM ".MyConfig::getValue("dbPrefix")."car_model cm ".$sqlJoin." WHERE cm.producer_id IN (SELECT otomoto_id FROM ".MyConfig::getValue("dbPrefix")."car_producer cr WHERE cr.producer_id IN (".implode(",",$producer_list).") ) ".$where_type.$group." ".$where_1."  ORDER BY cm.producer_id, cm.model_name";
