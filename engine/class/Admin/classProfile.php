...
class Profile {
	...
	public function zaloguj($login = '', $haslo = '', $redirect = 0) {
		...
		try{
			...
			$sql = " SELECT *
					FROM ".MyConfig::getValue("dbPrefix")."users
			...
			if(!is_array($tablicaWynikow) && count($tablicaWynikow) < 2)
			{
				...
				{
					if($wiersz['active'] == 0) {
						header("Location: ".MyConfig::getValue("wwwPatchPanel")."zaloguj.html,1,login_active_error");
						exit();
					}
                                          session_start();
		...
		if(count($tablicaBledow) > 0)
		{
			...
			$sql = "UPDATE ".MyConfig::getValue("dbPrefix")."users SET login_last_date = login_date, login_date = now() WHERE id_user = ".$idUzytkownikaBaza;
	...
	public function usunUsera($id_user=0) {
		if($id_user) {
			$pdo = new ConnectDB() ;
			$sql = "DELETE FROM ".MyConfig::getValue("dbPrefix")."users WHERE id_user = ".$id_user;
			$wynik = $pdo -> exec($sql);
			if($wynik>0)
				return true;
			else
	...
	public function getUserAjax($id_user=0) {
		if($id_user) {
			$pdo = new ConnectDB() ;
			$sql = "SELECT * FROM ".MyConfig::getValue("dbPrefix")."users WHERE id_user = ".$id_user." ORDER BY login";
			$wynik = $pdo -> query($sql);
			$tablicaWynikow = $wynik -> fetchAll();

			$sql = "SELECT * FROM ".MyConfig::getValue("dbPrefix")."users_grupy ORDER by nazwa";
	...
	public function saveUserAjax($id_user) {
		if($id_user) {
			$pdo = new ConnectDB() ;
			$sql = "UPDATE ".MyConfig::getValue("dbPrefix")."users SET login = :login, imie = :imie, nazwisko = :nazwisko, email = :email, id_grupy = :id_grupy WHERE id_user=".$id_user;
			...
			$sql = "SELECT * FROM ".MyConfig::getValue("dbPrefix")."users u LEFT JOIN users_grupy ug ON u.id_grupy = ug.id_grupy WHERE id_user = ".$id_user." ORDER BY login";
			$wynik = $pdo -> query($sql);
			$tablicaWynikow = $wynik -> fetchAll();
	...
	public function changeUserAjax($id_user) {
		if($id_user) {
			$pdo = new ConnectDB() ;
			$sql = "SELECT active FROM ".MyConfig::getValue("dbPrefix")."users WHERE id_user=".$id_user;
			$wynik = $pdo -> query($sql);
			$tablicaWynikow = $wynik -> fetchAll();
			if($tablicaWynikow[0]['active'] == 1) {
				$sql = "UPDATE ".MyConfig::getValue("dbPrefix")."users SET active = 0 WHERE id_user=".$id_user;
				$pdo -> query($sql);
				echo MyConfig::getValue("wwwPatch").'img/admin/main_off.jpg';
			}
			else {
				$sql = "UPDATE ".MyConfig::getValue("dbPrefix")."users SET active = 1 WHERE id_user=".$id_user;
				$pdo -> query($sql);
	...
	public function dopiszUzytkownika($imie, $nazwisko, $firma, $nip, $ulica, $kod, $miasto, $email, $telefon)
	{
		...
		try {
			...
			$sql = "INSERT INTO ".MyConfig::getValue("dbPrefix")."users (imie, nazwisko, login, haslo, miasto, email, telefon)
	...
	public function pobierzListeUserow() {
		...
		$pdo = new ConnectDB() ;
		$sql = "SELECT * FROM ".MyConfig::getValue("dbPrefix")."users u LEFT JOIN ".MyConfig::getValue("dbPrefix")."users_grupy ug ON u.id_grupy = ug.id_grupy ORDER BY login";
		$wynik = $pdo -> query($sql);
		$tablicaWynikow = $wynik -> fetchAll();
		...
		if($aPost['haslo'] != '') {
			if($aPost['haslo_new'] != '' && $aPost['haslo_new_2'] != '' && $aPost['haslo_new'] == $aPost['haslo_new_2']) {
				...
				if(count($tablicaWynikow) == 1) {
					...
					$sql = "UPDATE ".MyConfig::getValue("dbPrefix")."users SET haslo = '".$haslo_crypt_new."' WHERE id_user = ".$tablicaWynikow[0]['id_user'];
					$wynik2 = $pdo -> exec($sql);
					if($wynik2 > 0)
						return true;
					else
