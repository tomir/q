 ...
 **/
 jQuery.fn.jqUploader = function(options) {
    return this.each(function(index) {
		...
		var $thisInput = $("#upload_field");
		var $thisLabel = $("#upload_label");
		...
		TerminateJQUploader = function(containerId,filename,varname){
			...
			$this.text('').append('<div style="margin:10px;"><span style="color:#00CC00">Plik <strong>'+filename+'</strong> zostal pomyslnie wgrany. Uzupelnij pozostale pola i kliknij "zapisz".</span></div><input name="'+varname+'" type="hidden" id="'+varname+'" value="'+filename+'"/>');
			var myForm = $this.parents("form");
			myForm.submit(function(){return true});
			$("#nazwaZdjecia").val(filename);
			$("#zapisz").show();
		}
		var myParams = '';
