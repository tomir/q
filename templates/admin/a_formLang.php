    ...
    <?php if($aPola) {
      	...
      	<div class="box-container rounded_by_jQuery_corners" style="-moz-border-radius-bottomleft: 5px; -moz-border-radius-bottomright: 5px;">
			<form class="middle-forms" style="padding: 10px;" id="myFromularz" name="formularz" action="<?php echo $adminPatch; ?><?php echo $akcja; ?>,<?php echo $akcja2; ?>,<?php echo $formId; if($akcja4 != '') echo ','.$akcja4; ?>.html" method="post"><input type="hidden" name="zapisz" value="1" />
				...
				<div id="tformtabs">
					<p style="width: 90px; float: left; font-weight: bold; margin: 5px;">Wybierz język:</p>
					<ul style="display: inline;">
					<?php foreach($aLangs as $lang) { ?>
						<li style="display: inline;"><a href="#lang_<?php echo $lang['prefix']; ?>"><img style="margin: 5px;" src="../images/admin/flags/<?php echo $lang['prefix']; ?>.jpg" alt="<?php echo $lang['prefix']; ?>" /></a></li>
					<?php } ?>
					</ul>
					<br style="clear: both;" />
				<?php foreach($aLangs as $lang) { ?>
					<div id="lang_<?php echo $lang['prefix']; ?>">
						<div style="margin-top: 15px;font-size: 12px; color: #23AAAA;">Treść dla języka: <b><?php echo $lang['name']; ?></b></div>
						<fieldset>
							<ol>
								<?php $class = "even"; foreach($aPola as $row_p) {
									if(($lang['prefix'] != 'pl' && $row_p['lang'] == 1) || $lang['prefix'] == 'pl') {
									...
									<?php if($row_p['typ_pola'] == "text" || $row_p['typ_pola'] == "password") { ?>
										...
										<label><input class="txtbox-long" style="<?php echo $row_p['styl']; ?>" type="<?php echo $row_p['typ_pola']; ?>" name="<?php if($row_p['lang'] == 1) echo $row_p['column_s'].'_field['.$lang['prefix'].']'; else echo $row_p['column_s'].'_field'; ?>" id="<?php if($row_p['lang'] == 1) echo $row_p['column_s'].'_field['.$lang['prefix'].']'; else echo $row_p['column_s'].'_field'; ?>" value="<?php if($row_p['lang'] == 1) echo stripslashes($aDane[0][$colum][$lang['prefix']]); else echo stripslashes($aDane[0][$colum]); ?>" /></label>
									<?php } elseif($row_p['typ_pola'] == "textarea") { ?>
										<label><textarea <?php if($row_p['textarea_tinymc'] == 1) echo 'class="ckeditor"'; ?> id="<?php if($row_p['lang'] == 1) echo $row_p['column_s'].'_field['.$lang['prefix'].']'; else echo $row_p['column_s'].'_field'; ?>" style="width: <?php echo $row_p['textarea_x']; ?>px; height: <?php echo $row_p['textarea_y']; ?>px; <?php echo $row_p['styl']; ?>" name="<?php if($row_p['lang'] == 1)  echo $row_p['column_s'].'_field['.$lang['prefix'].']'; else echo $row_p['column_s'].'_field'; ?>" ><?php if($row_p['lang'] == 1) echo stripslashes($aDane[0][$colum][$lang['prefix']]); else echo stripslashes($aDane[0][$colum]); ?></textarea></label>
									<?php } elseif($row_p['typ_pola'] == "select" && $lang['prefix'] == 'pl') { ?>
									...
									<?php } elseif($row_p['typ_pola'] == "checkbox" && $lang['prefix'] == 'pl') { ?>
